    ...
    public class TheHamBizServices : ITheHamBizServices
    {
        ...
        public async Task<int> AddInCarSerivce(RequestInTheHamBizDTO dto)
        {
            try
            {
                ...
                if (dto.IS_WAIT == "0") // 정기차량(입주민)
                {
                    result = await TheHamBizRepository.AddInCar(ParkingHistoryTB);
                    if (result > 0)
                        return 1;
                    else
                        return -1;
                }
                else if (dto.IS_WAIT == "1") // 방문차량, 블랙리스트
                {
                    result = await TheHamBizRepository.AddInCar(ParkingHistoryTB);
                    if (result > 0)
                        return 1;
                ...
                else if (dto.IS_WAIT == "2")
                {
                    var SelectPackingModel = await TheHamBizRepository.SelectIOSeqInfoAsync(dto.IO_SEQ).ConfigureAwait(false);
                    if (SelectPackingModel is not null && SelectPackingModel.IsWait == "1")
                    {
                        // 모델 엎어쳐야함.
                        SelectPackingModel.ParkId = dto.PARK_ID; // 주차장ID

                        SelectPackingModel.IsWait = dto.IS_WAIT;
